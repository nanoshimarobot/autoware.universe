...
struct PointXYZIE
{
  ...
  friend bool operator==(const PointXYZIE & p1, const PointXYZIE & p2) noexcept
  {
    return float_eq<float>(p1.x, p2.x) && float_eq<float>(p1.y, p2.y) &&
           float_eq<float>(p1.z, p2.z) && float_eq<float>(p1.intensity, p2.intensity) &&
           float_eq<float>(p1.entity_id, p2.entity_id);
  }
};
...
LIDAR_UTILS__DEFINE_FIELD_GENERATOR_FOR_MEMBER(entity_id);
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
  autoware_point_types::PointXYZIE,
  (float, x,
   x)(float, y, y)(float, z, z)(float, intensity, intensity)(std::int32_t, entity_id, entity_id))
